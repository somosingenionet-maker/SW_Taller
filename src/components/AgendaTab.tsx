...
import { getNumerosOT, listOrdenesPorRangoRecepcion, listOrdenesActivas, OrdenAgendaRow } from '../lib/data/ordenes';
import { NuevoCliente } from '../lib/data/clientes';
...
interface Props {
  ...
  onAddCliente: (c: NuevoCliente) => Promise<Cliente>;
}
...
/** Solo para etiquetar las entradas al taller que se muestran en la Agenda — el detalle completo de cada OT vive en Taller. */
const OT_ESTADO_META: Partial<Record<OTEstado, { label: string; color: string; bg: string }>> = {
...
export default function AgendaTab({ citas, vehiculos, clientes, onAddCita, onUpdateCita, onDeleteCita, onCreateOT, onAddCliente }: Props) {
  ...
  // Cuántos vehículos hay ahora mismo en el taller (recibido/en reparación/
  // listo) — un recordatorio fijo de la carga actual mientras se agenda,
  // no una predicción por día (no se sabe cuándo saldrá cada uno).
  const [enTallerCount, setEnTallerCount] = useState<number | null>(null);
  useEffect(() => {
    listOrdenesActivas().then((ots) => setEnTallerCount(ots.length)).catch(() => setEnTallerCount(null));
  }, []);
  ...
  const monthAncla = selectedDay;
  const monthDays = useMemo(() => {
    const inicioGrid = startOfWeek(startOfMonth(monthAncla));
    return Array.from({ length: 42 }, (_, i) => addDays(inicioGrid, i));
  }, [monthAncla]);

  // Rango de días realmente visible según la vista — para no pedir OTs de
  // fuera de lo que se está mostrando.
  const rangoVisible = useMemo((): [string, string] => {
    if (vista === 'dia') return [localKey(selectedDay), localKey(selectedDay)];
    if (vista === 'semana') return [localKey(weekDays[0]), localKey(weekDays[6])];
    return [localKey(monthDays[0]), localKey(monthDays[41])];
  }, [vista, selectedDay, weekDays, monthDays]);
  ...
  const handleSave = async () => {
    ...
    if (form.modo === 'nuevo' && (!form.contactoNombre.trim() || !form.contactoApellidos.trim())) {
      setFormError('Indica el nombre y los apellidos del cliente.');
      return;
    }

    setSaving(true);
    try {
      const citaExistente = editingId ? citas.find((c) => c.id === editingId) : undefined;

      // "Cliente nuevo" ya no se queda en un simple nombre suelto en la
      // cita: se crea el cliente de verdad (solo con lo esencial — el NIF
      // y la dirección se completan más tarde) para que quede en el CRM,
      // con su propio historial y portal. Si ya se había creado antes (al
      // reeditar la cita sin cambiar de modo), no se vuelve a crear.
      let clienteId = form.modo === 'registrado' ? form.clienteId : citaExistente?.clienteId;
      if (form.modo === 'nuevo' && !clienteId) {
        const creado = await onAddCliente({
          nombre: form.contactoNombre.trim(),
          apellidos: form.contactoApellidos.trim(),
          nifNiePasaporte: '',
          correo: '',
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        {enTallerCount !== null && (
          <div
            className="flex items-center gap-2 bg-orange-50 border border-orange-100 text-orange-700 rounded-2xl px-3.5 py-2"
            title="Vehículos recibidos, en reparación o listos para entregar — no incluye lo que aún no ha entrado"
          >
            <Wrench className="w-3.5 h-3.5" />
            <span className="text-sm font-extrabold">{enTallerCount}</span>
            <span className="text-xs font-semibold">en el taller ahora</span>
          </div>
        )}
      </div>
      ...
      {(() => {
        ...
        return (
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              <p className="text-sm font-bold text-slate-800 flex items-center gap-1.5"><Wrench className="w-3.5 h-3.5 text-slate-400" /> Entradas al taller</p>
              <span className="text-xs text-slate-400">{ots.length} vehículo{ots.length !== 1 ? 's' : ''}</span>
            </div>
            <div className="divide-y divide-slate-50">
              {ots.map((o) => {
                const meta = OT_ESTADO_META[o.estado];
                return (
                  <div key={o.id} className="px-5 py-3 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                      <span className="text-xs font-mono font-bold text-slate-400 shrink-0">{o.numero}</span>
                      <span className="text-sm text-slate-700 truncate">{o.vehiculoMarca} {o.vehiculoModelo} · {o.vehiculoMatricula}</span>
                      <span className="text-xs text-slate-400 truncate">{o.clienteNombre} {o.clienteApellidos}</span>
                    </div>
                    {meta && <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ${meta.bg} ${meta.color}`}>{meta.label}</span>}
                  </div>
                );
              })}
            </div>
              ...
              {form.modo === 'registrado' ? (
                ...
                <>
                  <p className="text-[11px] text-slate-400 -mt-1">
                    Se crea el cliente en el CRM con estos datos — el NIF y la dirección se pueden completar después.
                  </p>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                    ...
                    <div>
                      ...
                      <input type="text" value={form.contactoApellidos} onChange={(e) => setForm((f) => ({ ...f, contactoApellidos: e.target.value }))}
                        className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
                    </div>
                  </div>
                  <div>
...
function VistaSemanal({ weekDays, citas, otsRecepcion, tecnicos, hoy, clienteLabel, onSelectCita, onSelectDay }: VistaSemanalProps) {
        ...
        <div className="grid grid-cols-7 gap-2 min-w-[770px]">
          {weekDays.map((d, i) => {
            ...
            const citasDia = citas
              .filter((c) => localKey(new Date(c.fechaHora)) === key)
              .sort((a, b) => a.fechaHora.localeCompare(b.fechaHora));
            const otsDia = otsRecepcion.filter((o) => o.fechaRecepcion === key);
            return (
              <div key={key} className="flex flex-col bg-slate-50/60 border border-slate-100 rounded-2xl overflow-hidden">
                ...
                <div className="flex-1 p-1.5 space-y-1.5 min-h-[90px]">
                  ...
                  {otsDia.length > 0 && (
                    <div
                      className="w-full px-2 py-1.5 rounded-xl text-[10px] leading-tight bg-orange-50 text-orange-700 font-semibold flex items-center gap-1"
                      title={otsDia.map((o) => `${o.numero} · ${o.vehiculoMarca} ${o.vehiculoModelo} (${o.clienteNombre} ${o.clienteApellidos})`).join('\n')}
                    >
                      <Wrench className="w-3 h-3 shrink-0" /> {otsDia.length} entrada{otsDia.length !== 1 ? 's' : ''} al taller
                    </div>
                  )}
                </div>
              </div>
...
function VistaMensual({ mesAncla, citas, otsRecepcion, hoy, clienteLabel, onSelectCita, onSelectDay }: VistaMensualProps) {
  const dias = useMemo(() => {
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {otsDia.length > 0 && (
                    <span
                      className="flex items-center gap-0.5 text-[9px] font-bold text-orange-600 bg-orange-50 rounded-full px-1.5 py-0.5"
                      title={otsDia.map((o) => `${o.numero} · ${o.vehiculoMarca} ${o.vehiculoModelo}`).join('\n')}
                    >
                      <Wrench className="w-2.5 h-2.5" /> {otsDia.length}
                    </span>
                  )}
                </div>
