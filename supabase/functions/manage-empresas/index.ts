  ...
  try {
    if (action === 'create_empresa') {
      ...
      const { data: empresa, error: empresaErr } = await admin
        .from('empresas')
        .insert({ nombre, nif })
        .select('id, nombre, tagline, razon_social, nif, direccion_fiscal, correo, telefono, web, ciudad, brand_color, logo_base64, logo_url, activo, recordatorios_automaticos_activos, plantillas_recordatorios, factura_prefijo, siguiente_numero_factura')
        .single();
